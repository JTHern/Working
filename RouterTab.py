        ...
        if self.device['device_type'] == 'cisco_ios_serial':
            try:
                ...
                if version >= 15.4:
                    ...
                    self.signal.emit('ERROR: Old version of IOS Detected\n Correct version may be in flash:')
                ...
                for p in ports:  # may just automate this in the future using this method if only one is found
            ...
            try:
                ...
                if version >= 15.4:
                    ...
                    self.signal.emit('Version below 15.4, correct version may be in flash:')
                    self.signal.emit(show_flash)
                    self.signal.emit('===============\n'
                                     'Ensure the config you plan to load is compatible with'
                                     ' the version of IOS you are using. If not contact the help desk.')
                telnet.disconnect()
...
class RouterInfo(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        '''The output screen'''
        self._log_viewer = QPlainTextEdit(whatsThis="This displays the messages generated.", readOnly=True)
